...
IF EXISTS (SELECT 1 FROM Employee WHERE EID = 102)

BEGIN
    UPDATE Employee
    SET Salary = 50000
    WHERE EID = 101;
	Print 'Employee updated successfuly'
END
ELSE
	Print 'Employee with EID not found'

Select * from Employee

--Delete with IF EXISTS 

IF EXISTS (SELECT 1 FROM Employee WHERE EID = 103)
BEGIN
...
--Using IF EXISTS
IF NOT EXISTS (SELECT 1 FROM Employee WHERE EID = 101)
BEGIN
    INSERT INTO Employee (EID, Name, Salary)
    VALUES (101, 'Suresh', 5000);
...
Select EID, Ename, ISNULL(Email,'Pending') from employee

--CTE (Common table expression) -temporary result set

WITH CTE_HighSalaryEmployee

AS (SELECT EID, EName, Salary
    FROM Employee
    WHERE Salary > 50000)

SELECT * FROM CTE_HighSalaryEmployee;
