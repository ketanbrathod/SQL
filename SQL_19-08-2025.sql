...
Create Index IX_EMP1_Salary
On EMP1 (Salary ASC)
-- Drop Index <Tablename.indexname>
Drop Index EMP1.IX_EMP1_DepSalary
sp_Helpindex 'EMP1'
Create table Test1
(TestID INT,
...
Select * from Test1
Drop table test1
Create nonclustered Index IX_EMP1_DepSalary
On EMP1 (Salary DESC)
Create clustered Index IX_EMP1_Ename
On EMP1 (EID)
...
Select * from EMP1
Where Department IN ('QA', 'AI')

Create Unique index IX_EMP1_EID
On EMP1 (EID)
Select * from EMP1
...
• Quantity INT
• Price Decimal (10,2)
• ShippingAddress varhcar (255)
Create a table named Customers with the following columns:
• CustomerID INT PK Indentity(1,1)
...
SUM(o.Quantity * o.price) as Highest_Spending
from Customer c
join ORders o
...
Select c.State, 
AVG (o.Quatity * o.Price) as Average_ORder_Values
From Customer c
JOin Orders O
ON C.CustomerID = O.CustomerID
Group by c.state
...
Select P.Productnames
From Product p
Join Orders O
ON p.Product ID = o.ProductID
Group by p.Productnames
having Count(o.ProductID) >1

--6. Find the products that have been ordered by the most customers.
Select P.Productnames, o.CustomerID
From Product p
Join Orders O
ON p.Product ID = o.ProductID
Group by p.Productnames
...
NOT IN (Select Distinct ProductID from 
orders)

-- Stored Procedure
--Wrapping a query or hidding the query into a specific name
...
-- Type 2 of SP (with Input parameter)
Select * from EMP1
...
Begin
Select * from EMP1
Where EID = @EID AND Department = @Department
End

spEmpDetails @Department = 'Cloud', @EID=3

sp_helptext spEmpDetails

SP_help spEmpDetails

SP_depends spEmpDetails

-- Advantages (Reusability, Performance, Security, Reduces Network Traffic )
-- Type 3 (using Output parameter)

Create Procedure spGetCountEmpDept
@dept varchar(20),
@Empcount INT Out
AS
Begin
	Select @Empcount = Count(EID) from EMP1 where department = @dept
End

Declare @TotalCount INT 
Execute spGetCountEmpDept 'cloud', @TotalCount Output
Print @TotalCount
...
As
Begin
	Select @EmpCount =count(EID) from EMP1
End

Declare @Total INT
Exec sp_GetAllEmp @Total Output
Print @Total
